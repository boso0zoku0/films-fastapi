...
from tests.test_api.conftest import create_film_exists, film_create_random_slug
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                ...
                id="no desc to max desc"),
        ],
        indirect=["film"]
    )
    
    def test_film_update_parsial(self, new_description: str, auth_client: TestClient, film: FilmsCreate) -> None:
...
class TestUpdateFilms:
    ...
    def test_update_movie_details(self, auth_client: TestClient, film: FilmsCreate, new_description: str, new_year_release: int) -> None:
        url = app.url_path_for("put_film", slug=film.slug)
        new_film = FilmsCreate(name="some-name", slug=film.slug, description = new_description, year_release=new_year_release)
